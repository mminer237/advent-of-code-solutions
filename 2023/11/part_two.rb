...
galaxies.map! { |galaxy| [
	galaxy[0] + (empty_columns.select.with_index { |empty, x| empty && x < galaxy[0] }).length * (1_000_000 - 1),
	galaxy[1] + (empty_rows.select.with_index { |empty, y| empty && y < galaxy[1] }).length * (1_000_000 - 1)
]}
